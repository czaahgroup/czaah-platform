...
export default function PartnerMailPage() {
  ...
  const [threadInfo, setThreadInfo] = useState<{ subject: string; externalAddress: string; mailboxAddress: string } | null>(null)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mailbox-messages-${selectedId}`)
      .on(
        'postgres_changes',
  ...
  return (
    <div style={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
      <h1 style={{ fontFamily: "'Cinzel', serif", fontSize: '24px', color: '#fff', margin: '0 0 16px' }}>Mail</h1>
