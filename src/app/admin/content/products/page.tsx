...
export default function ProductsPage() {
  ...
  if (loading) {
    return <div className="text-on-surface-variant py-12 text-center">Loading products...</div>
  }

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-[family-name:var(--font-heading)] text-2xl text-on-surface">Products</h1>
        <button
          onClick={openAdd}
          ...
          <div className="bg-surface-container-low border border-outline-variant/10 rounded-none w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-on-surface-variant mb-1.5">Sector</label>
                  <select
                ...
                <div>
                  <label className="block text-sm text-on-surface-variant mb-1.5">Service</label>
                  <select
                ...
                <div className="flex flex-col justify-end gap-2 pb-1">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded-noneborder-outline-variant/10 accent-primary"
                    />
                    <span className="text-sm text-on-surface-variant">Enquiries Enabled</span>
                  </label>
                </div>
              </div>
            </div>
      ...
      {products.length === 0 ? (
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-none px-6 py-16 text-center">
          <p className="text-on-surface-variant">No products yet. Add your first product to get started.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-outline-variant/10">
                  ...
                  <th className="text-left px-5 py-3 text-on-surface-variant font-medium text-xs uppercase tracking-wider">Service</th>
              ...
              <tbody>
                {products.map((product, idx) => (
                  <tr key={product.id} className="border-b border-outline-variant/10/50 hover:bg-surface-container-lowest/30 transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-on-surface-variant/50 ml-1">{product.display_order}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-on-surface font-medium">{product.name}</span>
                      {product.is_enquiry_enabled && (
                        <span className="ml-2 text-[10px] bg-primary/20 text-primary px-1.5 py-0.5 rounded">ENQ</span>
                      )}
                    </td>
                    <td className="px-5 py-3 text-on-surface-variant/50 font-mono text-xs">{product.slug}</td>
                    <td className="px-5 py-3 text-on-surface-variant text-xs">{product.sectors?.name || '--'}</td>
                    <td className="px-5 py-3 text-on-surface-variant text-xs">{product.services?.name || '--'}</td>
                    <td className="px-5 py-3">
                      <button
