...
export async function POST(request: NextRequest) {
  try {
    ...
    // inviteUserByEmail is an implicit-flow link — the session tokens come back
    // in the URL hash, which /reset-password consumes directly (not via
    // /api/auth/callback: the hash never reaches the server). Without
    // redirectTo, Supabase falls back to the Site URL (the homepage).
