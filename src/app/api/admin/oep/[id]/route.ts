...
export async function PATCH(
  ...
  try {
    ...
    if (body.status) {
      ...
      const validStatuses = ['registered', 'contacted', 'verified', 'inactive']
        ...
        if (existing?.profile_id) {
          ...
          if (linkedProfile?.status !== 'approved') {
            return NextResponse.json(
              { error: 'Cannot approve — this promoter\'s account has not passed KYC review yet.' },
              { status: 400 }
            )
          }
          ...
          return NextResponse.json(
            { error: 'Cannot approve — no identity document on file for this promoter.' },
            { status: 400 }
          )
        }
      }
    ...
    if (error) {
      console.error('OEP update error:', error)
    ...
    console.error('PATCH /api/admin/oep/[id] error:', err)
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await adminClient.from('oep_registry').delete().eq('id', id)

    if (error) {
      console.error('OEP delete error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('DELETE /api/admin/oep/[id] error:', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
