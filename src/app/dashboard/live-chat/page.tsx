...
export default function LiveChatPage() {
  ...
  const [chatDetails, setChatDetails] = useState<{ admin_name: string; member_name: string } | null>(null)
  ...
  // Load admins for new chat picker
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`direct-messages-${selectedChatId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  // Also subscribe to all chats for unread badge updates
  useEffect(() => {
    const channel = supabase
  ...
  async function handleNewChat(adminId: string) {
    ...
    try {
      ...
      if (res.ok && json.data?.id) {
        ...
        console.error('Failed to create chat:', json.error || 'Unknown error')
      }
    } catch (err) {
                ...
                <div style={{
                  ...
                  {admins.length === 0 ? (
                    ...
                    admins.map((admin) => (
                      <button
                        key={admin.id}
                        onClick={() => handleNewChat(admin.id)}
                        style={{
                          display: 'block',
                          width: '100%',
                          padding: '10px 16px',
            ...
            {loading ? (
              ...
              <div style={{
                ...
                <p style={{
                  ...
                  Start a conversation with an admin using the button above
                </p>
              </div>
            ) : (
              chats.map((chat) => (
          ...
          {!selectedChatId ? (
            <div style={{
              ...
              <p style={{
                ...
                Or start a new chat with an admin
              </p>
            </div>
          ) : (
            <>
              ...
              <div style={{
                ...
                <div>
                  <h2 style={{
                    ...
                    {chatDetails?.admin_name || selectedChat?.admin_name || 'Chat'}
                  </h2>
                  <span style={{
                    fontFamily: "'Raleway', sans-serif",
                    fontSize: '11px',
                    ...
                    {selectedChat?.admin_role?.replace(/_/g, ' ') || 'Admin'}
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {selectedChat && userId && (
                    <>
                      <button
                        onClick={() => {
                          const targetId = selectedChat.admin_id
                          const targetNm = chatDetails?.admin_name || selectedChat.admin_name || 'Admin'
                  ...
                  <Link
                    href={`/dashboard/live-chat/${selectedChatId}`}
                    style={{
                      fontFamily: "'Raleway', sans-serif",
                      fontSize: '11px',
                      color: 'rgba(255,255,255,0.3)',
