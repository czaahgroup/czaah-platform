...
export default function VerifyOEPPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/public/oep/verify/${id}`);
        if (!res.ok) {
          setNotFound(true);
          return;
        }
      ...
      >
        <div style={{ padding: '28px 28px 20px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '3px', color: 'rgba(255,255,255,0.35)', marginTop: '6px' }}>
            EMPLOYMENT PROMOTER VERIFICATION
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>
                This reference does not match any record in CZAAH&apos;s Employment Promoter database.
              </p>
            </div>
          ) : (
            <>
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                <div style={{
                  ...
                  <span className="material-symbols-outlined" style={{ fontSize: '40px', color: 'rgba(201,168,76,0.6)' }}>flight_takeoff</span>
                </div>
                <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '20px 0', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Head Office</div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>{data.head_office_location}</div>
                </div>
                <div>
                ...
                <div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Years Active</div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>{data.years_in_operation}</div>
                </div>
              </div>
              ...
              <p style={{ fontFamily: "'Raleway', sans-serif", fontSize: '11px', color: 'rgba(255,255,255,0.3)', lineHeight: 1.7, marginTop: '20px', textAlign: 'center' }}>
                This is an official verification record from CZAAH Group&apos;s partner network database. For partnership enquiries, contact us at czaah.com.
              </p>
            </>
          )}
        </div>
