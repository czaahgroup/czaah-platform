...
export default function VerifyMemberPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/public/member/verify/${id}`);
        if (!res.ok) {
          setNotFound(true);
          return;
        }
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>
                This reference does not match any active CZAAH membership.
              </p>
            </div>
          ) : (
            <>
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                <div style={{
                  ...
                  <span className="material-symbols-outlined" style={{ fontSize: '40px', color: 'rgba(201,168,76,0.6)' }}>badge</span>
                </div>
                <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '20px 0', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Tier</div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>{data.tier}</div>
                </div>
                <div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Member ID</div>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>{data.member_id}</div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Member Since</div>
              ...
              <p style={{ fontFamily: "'Raleway', sans-serif", fontSize: '11px', color: 'rgba(255,255,255,0.3)', lineHeight: 1.7, marginTop: '20px', textAlign: 'center' }}>
                This is an official verification record from CZAAH Group&apos;s membership database. For enquiries, contact us at czaah.com.
              </p>
            </>
          )}
        </div>
